...
class GaussianDiffusion:
    ...
    def training_losses_seq2seq(self, model, x_start, t,model_kwargs=None, noise=None):
        ...
        input_ids_x = model_kwargs.pop('input_ids').to(th.float).to(t.device) #seq对应的字典序号
        # input_ids_mask = model_kwargs.pop('input_mask').to(t.device) # seq中条件部分和生成部分
        
        
        if self.num_props:
            # props=input_ids_x[:,:num_props]
            ...
            x_start_mean = model.model.module.get_embeds(input_ids_x) #Transformer中的embed层，之前的完全不需要，可以删除
        ...
        x_t = self.q_sample(x_start, t, noise=noise, mask=input_ids_mask) # reparametrization trick. 重参数化，就是进行t时刻采样
        ...
        target = x_start
        model_output = model(x_t, self._scale_timesteps(t), **model_kwargs) #根据采样后结果，进行开始预测
        ...
        t0_mask = (t == 0)
        t0_loss = mean_flat((x_start_mean - model_out_x_start) ** 2) #这是真实和预测的均值
        ...
        if self.num_props:
            decoder_nll = self._token_discrete_loss(x_start[:,1:,:], get_logits, input_ids_x[:,self.num_props:].to(th.long)) # embedding regularization
            terms["nll"] = self._token_discrete_loss(model_out_x_start[:,1:,:], get_logits, input_ids_x[:,self.num_props:].to(th.long), mask=input_ids_mask[:,1:], truncate=True, t=t) # x_0->model_out_x_start
        else:
            decoder_nll = self._token_discrete_loss(x_start, get_logits, input_ids_x.to(th.long)) # embedding regularization
            terms["nll"] = self._token_discrete_loss(model_out_x_start, get_logits, input_ids_x.to(th.long), mask=input_ids_mask, truncate=True, t=t) # x_0->model_out_x_start
